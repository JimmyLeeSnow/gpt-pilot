...
class Project:
    ...
    def get_all_coded_files(self, relevant_files=None):
        ...
        if relevant_files:
            ...
            print(f"[get_all_coded_files] reduced context from {n_files0} to {n_files1} files, using: {rel_txt}")
    ...
    def save_file(self, data):
        ...
        description = describe_file(relpath(abspath(full_path), abspath(self.root_path)), data['content'])
    ...
    def save_files_snapshot(self, development_step_id, summaries=None):
        ...
        for file in files:
            ...
            if (created or self.skip_steps) and not file_in_db.description and file['content']:
                file_in_db.description = describe_file(relpath(abspath(file['full_path']), abspath(self.root_path)), file['content'])
                file_in_db.save()
